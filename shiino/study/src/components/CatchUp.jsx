...
function CatchUp({
  ...
  const message = () => {
    if (subjects.length === 0)
      return "科目を足すと、ここで進捗度をくらべられるよ";
    if (subjects.length === 1)
      return "科目が1つだけだから、まだくらべる相手がいないね";
    if (leveled) {
      if (average >= GOAL)
        return `全科目 ${GOAL}%！もう言うことないよ。好きな科目を好きなだけやろう`;
      const lead = isDisliked(recommended)
        ? `嫌いな ${recommended.name}`
        : recommended.name;
  ...
  return (
    <section className="card catch-up">
              ...
              <li key={subject.id} className="subject-bar">
                ...
                {skillsOf(subject.id, skills).length > 0 && (
                  <span className="skill-note">
                    やること{" "}
                    {skillsOf(subject.id, skills).filter((k) => k.done).length}{" "}
                ...
                <span className="bar-track catch-up-track">
                  ...
                  />
                  <span
                    className="leader-line is-target"
                    style={{ left: `${GOAL}%` }}
                  />
                  {!leveled && leader && gap > 0 && (
                    <span
                      ...
                      style={{
                        left: `${normalizeConfidence(leader.confidence)}%`,
                      }}
                    />
                  )}
                </span>
      ...
      {recommended &&
        subjects.length > 1 &&
        (leveled ? average < GOAL : gapSpread > 0) && (
